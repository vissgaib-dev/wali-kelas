const K={siswa:"dataSiswaKelas5",absensi:"absensiKelas5",jurnal:"jurnalKelas5",catatan:"catatanKelas5",nilai:"nilaiKelas5",setting:"settingKelas5"};
let siswa=baca(K.siswa,[]),absensi=baca(K.absensi,{}),jurnal=baca(K.jurnal,[]),catatan=baca(K.catatan,[]),nilai=baca(K.nilai,[]),setting=baca(K.setting,{wali:"Wali Kelas 5",kelas:"Kelas 5",sekolah:"",tahun:"2026/2027"}),editIndex=null;
...
$("formNilai").onsubmit=e=>{e.preventDefault();nilai.unshift({absen:Number($("nilaiSiswa").value),mapel:$("nilaiMapel").value.trim(),jenis:$("nilaiJenis").value.trim(),angka:Number($("nilaiAngka").value)});tulis(K.nilai,nilai);e.target.reset();isiPilihanSiswa();tampilkanNilai();toast("Nilai tersimpan")};
...
function isiPilihanSiswa(){const opsi=`<option value="">Pilih siswa</option>`+siswa.map(x=>`<option value="${x.absen}">${x.absen}. ${esc(x.nama)}</option>`).join("");$("catatanSiswa").innerHTML=opsi;$("nilaiSiswa").innerHTML=opsi}
...
function updateDashboard(){updateJumlah();const d=absensi[hariIni()]||{},v=Object.values(d);$("hadirHariIni").textContent=v.filter(x=>x==="Hadir").length;$("izinHariIni").textContent=v.filter(x=>x==="Sakit"||x==="Izin").length;$("alpaHariIni").textContent=v.filter(x=>x==="Alpa").length}
...
function eksporData(){const data={versi:1,tanggal:hariIni(),siswa,absensi,jurnal,catatan,nilai,setting};const blob=new Blob([JSON.stringify(data,null,2)],{type:"application/json"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`cadangan-wali-kelas-${hariIni()}.json`;a.click();URL.revokeObjectURL(a.href)}
$("fileImpor").onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{try{const d=JSON.parse(r.result);if(!confirm("Impor akan mengganti data aplikasi saat ini. Lanjut?"))return;siswa=d.siswa||[];absensi=d.absensi||{};jurnal=d.jurnal||[];catatan=d.catatan||[];nilai=d.nilai||[];setting=d.setting||setting;tulis(K.siswa,siswa);tulis(K.absensi,absensi);tulis(K.jurnal,jurnal);tulis(K.catatan,catatan);tulis(K.nilai,nilai);tulis(K.setting,setting);location.reload()}catch{alert("File cadangan tidak valid.")}};r.readAsText(f)};

tampilkanSiswa();tampilkanAbsensi();tampilkanRekap();tampilkanJurnal();tampilkanCatatan();tampilkanNilai();isiPilihanSiswa();muatPengaturan();updateDashboard();
